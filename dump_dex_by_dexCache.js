...
// src/dump_dex_by_dexCache_src.js
function untagPointer(ptrVal) {
  if (!ptrVal || ptrVal.isNull()) {
    return ptrVal;
  }
  var str = ptrVal.toString();
  var big = BigInt(str);
  big = big & 0x0000FFFFFFFFFFFFn;
...
function getFilesDir() {
  try {
    ...
    throw new Error("\u52A8\u6001\u83B7\u53D6\u5F53\u524D\u5E94\u7528\u79C1\u6709\u5B58\u50A8\u8DEF\u5F84\u5931\u8D25: " + e.message);
  }
  throw new Error("\u672A\u80FD\u5728\u5F53\u524D JVM \u5B9E\u4F8B\u4E2D\u5B9A\u4F4D\u5230\u6D3B\u52A8\u7684 Application Context");
}
function dumpAllDex() {
  return new Promise(function(resolve, reject) {
    frida_java_bridge_default.perform(function() {
      console.log("\n[dumpAllDex] === \u5F00\u59CB\u4E00\u952E\u904D\u5386 JVM \u5E76 Dump \u6240\u6709\u89E3\u5BC6 DEX (\u6781\u901F\u5806\u626B\u63CF\u7248) ===");
      var dumpDir = null;
      try {
        var FileJava = frida_java_bridge_default.use("java.io.File");
        var targetPath = getFilesDir() + "/all_dumped_dexs";
        var dir = FileJava.$new(targetPath);
        if (dir.exists()) {
          console.log("[dumpAllDex] \u68C0\u6D4B\u5230\u5DF2\u5B58\u5728\u65E7\u7684\u5B58\u50A8\u6587\u4EF6\u5939\uFF0C\u6B63\u5728\u6E05\u7406...");
          var deleteRecursive = function(file) {
            if (file.isDirectory()) {
          ...
          deleteRecursive(dir);
          console.log("[dumpAllDex] \u65E7\u5B58\u50A8\u6587\u4EF6\u5939\u6E05\u7406\u5B8C\u6210\u3002");
        }
        if (!dir.exists()) {
          dir.mkdirs();
        }
        dumpDir = dir.getAbsolutePath();
        console.log("[dumpAllDex] \u7EDF\u4E00\u5B58\u50A8\u6587\u4EF6\u5939\u5DF2\u5C31\u7EEA: " + dumpDir);
      } catch (e) {
        console.log("[dumpAllDex] \u521B\u5EFA\u5B58\u50A8\u76EE\u5F55\u5931\u8D25: " + e.message);
        reject(e.message);
        return;
      }
      var uniqueDexes = {};
      var dexFoundCount = 0;
      try {
        var dexFileField = null;
        var matchCount = 0;
        frida_java_bridge_default.choose("java.lang.DexCache", {
          onMatch: function(dexCache) {
            matchCount++;
            var prefix = "[dumpAllDex] [\u8BCA\u65AD #" + matchCount + "] ";
            try {
              if (!dexFileField) {
                dexFileField = dexCache.getClass().getDeclaredField("dexFile");
                dexFileField.setAccessible(true);
              }
              var dexFileVal = dexFileField.get(dexCache);
              if (!dexFileVal) {
                console.log(prefix + "dexFileVal \u5B57\u6BB5\u83B7\u53D6\u4E3A\u7A7A");
                return;
              }
              var dexFilePtr = longToPtr(dexFileVal);
              var rawStr = dexFileVal.toString();
              var location = "Unknown";
              try {
                var locationField = dexCache.getClass().getDeclaredField("location");
                locationField.setAccessible(true);
                var locationVal = locationField.get(dexCache);
                if (locationVal) {
                  location = locationVal.toString();
                }
              } catch (e) {
              }
              var verbose = matchCount <= 5 || location.indexOf("base.apk") !== -1;
              if (verbose) {
                console.log(prefix + "\u6B63\u5728\u5206\u6790: " + location);
                console.log(prefix + "  - \u539F\u59CB Java long \u5B57\u6BB5\u503C: " + rawStr);
                console.log(prefix + "  - \u8F6C\u6362\u540E\u65E0\u7B26\u53F7\u6307\u9488: " + dexFilePtr);
                console.log(prefix + "  - \u7ED3\u6784\u4F53 C++ \u5185\u5B58 Dump (8\u5B57\u8282\u4E00\u7EC4):");
                for (var i = 0; i < 8; i++) {
                  var offset = i * Process.pointerSize;
                  try {
                    var valPtr = dexFilePtr.add(offset).readPointer();
                    var valU32 = dexFilePtr.add(offset).readU32();
                    console.log(prefix + "    + \u504F\u79FB +" + offset + " (0x" + offset.toString(16) + ") => \u6307\u9488: " + valPtr + " | U32: " + valU32);
                  } catch (e) {
                    console.log(prefix + "    + \u504F\u79FB +" + offset + " (0x" + offset.toString(16) + ") => \u8BFB\u53D6\u5931\u8D25: " + e.message);
                  }
                }
              }
              if (dexFilePtr.isNull() || dexFilePtr.compare(ptr("0x1000")) < 0) {
                if (verbose) console.log(prefix + "  - [\u8FC7\u6EE4] \u6307\u9488\u4E3A\u7A7A\u6216\u6781\u5C0F\u810F\u5730\u5740");
                return;
              }
              var rangeFile = Process.findRangeByAddress(dexFilePtr);
              if (!rangeFile) {
                if (verbose) console.log(prefix + "  - [\u8FC7\u6EE4] Process.findRangeByAddress(" + dexFilePtr + ") \u672A\u627E\u5230\u53EF\u8BFB\u5185\u5B58\u8303\u56F4");
                return;
              } else {
                if (verbose) console.log(prefix + "  - \u5185\u5B58\u8303\u56F4\u5B58\u5728: " + rangeFile.base + " - " + rangeFile.base.add(rangeFile.size));
              }
              var dexBegin = null;
              var dexSize = 0;
              var ptrA = null;
              try {
                ptrA = dexFilePtr.add(Process.pointerSize).readPointer();
              } catch (errP) {
                if (verbose) console.log(prefix + "  - \u8BFB\u53D6 ptrA \u5F02\u5E38: " + errP.message);
              }
              if (verbose && ptrA) {
                console.log(prefix + "  - ptrA: " + ptrA + " (untagged: " + untagPointer(ptrA) + ")");
              }
              if (ptrA && isDexHeader(ptrA)) {
                dexBegin = ptrA;
                try {
                  dexSize = ptrA.add(32).readU32();
                  if (verbose) console.log(prefix + "  - \u65B9\u6848 A \u6210\u529F! dexBegin = " + dexBegin + ", dexSize = " + dexSize);
                } catch (errP) {
                }
              } else {
                if (verbose && ptrA) console.log(prefix + "  - ptrA \u6821\u9A8C isDexHeader \u5931\u8D25");
              }
              if (!dexBegin) {
                var ptrB = null;
                try {
                  ptrB = dexFilePtr.readPointer();
                } catch (errP) {
                  if (verbose) console.log(prefix + "  - \u8BFB\u53D6 ptrB \u5F02\u5E38: " + errP.message);
                }
                if (verbose && ptrB) {
                  console.log(prefix + "  - ptrB: " + ptrB + " (untagged: " + untagPointer(ptrB) + ")");
                }
                if (ptrB && isDexHeader(ptrB)) {
                  dexBegin = ptrB;
                  try {
                    dexSize = ptrB.add(32).readU32();
                    if (verbose) console.log(prefix + "  - \u65B9\u6848 B \u6210\u529F! dexBegin = " + dexBegin + ", dexSize = " + dexSize);
                  } catch (errP) {
                  }
                } else {
                  if (verbose && ptrB) console.log(prefix + "  - ptrB \u6821\u9A8C isDexHeader \u5931\u8D25");
                }
              }
              if (dexBegin && dexSize > 0) {
                var trueSize = dexSize;
                var untaggedDexBegin = untagPointer(dexBegin);
                var range = Process.findRangeByAddress(untaggedDexBegin);
                if (range) {
                  var availableSize = range.base.add(range.size).sub(untaggedDexBegin).toInt32();
                  if (availableSize > trueSize) {
                    trueSize = availableSize;
                  }
                }
                var baseStr = dexBegin.toString();
                if (!uniqueDexes[baseStr]) {
                  uniqueDexes[baseStr] = {
                    base: dexBegin,
                    size: trueSize,
                    location
                  ...
                  dexFoundCount++;
                  var sizeMB = (trueSize / 1024 / 1024).toFixed(2);
                  console.log("[dumpAllDex] [\u53D1\u73B0\u65B0 DEX] \u57FA\u5740: " + baseStr + " | \u957F\u5EA6: " + sizeMB + " MB (" + trueSize + " \u5B57\u8282) | \u8DEF\u5F84/\u6807\u8BC6: " + location);
                } else {
                  if (trueSize > uniqueDexes[baseStr].size) {
                    uniqueDexes[baseStr].size = trueSize;
                  }
                }
              } else {
                if (verbose) console.log(prefix + "  - [\u8FC7\u6EE4] \u65E0\u6CD5\u63D0\u53D6 dexBegin \u6216 dexSize \u4E3A 0");
              }
            } catch (innerErr) {
              console.log(prefix + "[\u626B\u63CF\u5F02\u5E38] \u5355\u4E2A DexCache \u5904\u7406\u5F02\u5E38: " + innerErr.message);
            }
          },
          onComplete: function() {
            console.log("[dumpAllDex] \u5185\u5B58\u5806\u626B\u63CF (DexCache) \u7ED3\u675F\u3002\u627E\u5230\u72EC\u7279 DEX \u6570\u91CF: " + dexFoundCount);
            console.log("\n[dumpAllDex] === \u5F00\u59CB\u5C06\u72EC\u7279\u7684\u5185\u5B58\u89E3\u5BC6 DEX \u6587\u4EF6\u4FDD\u5B58\u81F3\u8BBE\u5907\u672C\u5730\u76EE\u5F55 ===");
            var savedCount = 0;
            var totalSavedSize = 0;
            var keys = Object.keys(uniqueDexes);
            for (var j = 0; j < keys.length; j++) {
              var key = keys[j];
              var dex = uniqueDexes[key];
              try {
                var dexMB = (dex.size / 1024 / 1024).toFixed(2);
                var filename = "base";
                if (dex.location) {
                  var lastSlash = dex.location.lastIndexOf("/");
                  if (lastSlash !== -1) {
                    var baseName = dex.location.substring(lastSlash + 1);
                    if (baseName) {
                      filename = baseName;
                    }
                  } else if (dex.location.indexOf(".") !== -1 || dex.location.indexOf("/") === -1) {
                    filename = dex.location;
                  }
                }
                var destPath = dumpDir + "/" + filename + "_" + key + "_size_" + dexMB + "MB.dex";
                if (isDexHeader(dex.base)) {
                  var file = new File(destPath, "wb");
                  file.write(dex.base.readByteArray(dex.size));
                  file.flush();
                  file.close();
                  savedCount++;
                  totalSavedSize += dex.size;
                  console.log("[dumpAllDex] [\u4FDD\u5B58\u6210\u529F] \u6587\u4EF6 #" + savedCount + ": " + destPath + " (" + dexMB + " MB) (\u8DEF\u5F84: " + dex.location + ")");
                } else {
                  console.log("[dumpAllDex] [\u9B54\u6570\u6821\u9A8C\u5931\u8D25] \u8BE5\u5185\u5B58\u5757\u5DF2\u4E0D\u5305\u542B\u5408\u6CD5\u7684 DEX \u6587\u4EF6\u7ED3\u6784: " + key);
                }
              } catch (e) {
                console.log("[dumpAllDex] [\u4FDD\u5B58\u5F02\u5E38] \u57FA\u5740 " + key + " \u5199\u5165\u5931\u8D25: " + e.message);
              }
            }
            var totalSavedSizeMB = (totalSavedSize / 1024 / 1024).toFixed(2);
            console.log("\n============================================================");
            console.log("[dumpAllDex] \u5168\u5C40 DUMP \u5B8C\u6210\uFF01\u6210\u679C\u6C47\u62A5\uFF1A");
            console.log("      \u8BC6\u522B\u72EC\u7279 DEX \u6570\u91CF: " + keys.length);
            console.log("      \u6210\u529F\u4FDD\u5B58 DEX \u6587\u4EF6: " + savedCount);
            console.log("      \u6210\u529F\u4FDD\u5B58 DEX \u603B\u4F53\u79EF: " + totalSavedSizeMB + " MB");
            console.log("      \u8BBE\u5907\u7AEF\u4E34\u65F6\u6587\u4EF6\u5939: " + dumpDir);
            console.log("============================================================\n");
            send({
              type: "done",
              dumpDir
        ...
        console.log("[dumpAllDex] [\u5168\u5C40\u5F02\u5E38] \u904D\u5386 DexCache \u53D1\u751F\u4E25\u91CD\u9519\u8BEF: " + err.message);
        reject(err.message);
      }
    });
  });
